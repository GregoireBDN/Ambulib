...
HavRid - Application pour services d'ambulance (Turborepo monorepo):

- **API** (`apps/api`): NestJS backend with PostgreSQL/Prisma - ✅ COMPLET → [📖 API Documentation](apps/api/CLAUDE.md)
- **Mobile** (`apps/mobile`): Expo React Native application - ✅ COMPLET → [📱 Mobile Documentation](apps/mobile/CLAUDE.md)
- **Client** (`apps/client`): Next.js app for patients/guardians - ✅ AUTH RÉCENTE → [👥 Client Documentation](apps/client/CLAUDE.md)
- **Fleet** (`apps/fleet`): Next.js app for fleet managers - 🚧 EN DÉVELOPPEMENT → [🚗 Fleet Documentation](apps/fleet/CLAUDE.md)
- **Admin** (`apps/admin`): Next.js app for administrators - 🚧 EN DÉVELOPPEMENT → [⚙️ Admin Documentation](apps/admin/CLAUDE.md)
- **UI** (`packages/ui`): Shared React components with shadcn/ui - ✨ ÉTAT DE L'ART → [🎨 UI Documentation](packages/ui/CLAUDE.md)
- **Auth** (`packages/auth`): Shared authentication system - ✅ COMPLET → [🔐 Auth Documentation](packages/auth/CLAUDE.md)
...
pnpm storybook    # Start UI components Storybook
```
...
│   ├── ui/           # Shared Components (shadcn/ui + Accessible + Auth)
│   ├── auth/         # Authentication system (client + server + mobile)
