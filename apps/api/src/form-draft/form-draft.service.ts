...
export class FormDraftService {
  ...
  async saveDraft(
    ...
    this.logger.log(
      `Brouillon sauvegardé: ${draftId} (expire: ${expiresAt.toISOString()})`,
    );
  ...
  async getDraft(
    ...
    if (draft.sessionId !== sessionId) {
      this.logger.warn(
        `Tentative d'accès non autorisé au brouillon: ${draftId}`,
      );
      return null;
    }
  ...
  private cleanExpiredDrafts(): void {
    ...
    if (cleanedCount > 0) {
      this.logger.log(
        `Nettoyage automatique: ${cleanedCount} brouillons expirés supprimés`,
      );
    }
  }
  ...
  private cleanDraftsBySession(sessionId: string): void {
    ...
    if (cleanedCount > 0) {
      this.logger.log(
        `Session ${sessionId}: ${cleanedCount} anciens brouillons supprimés`,
      );
    }
  }
